...
### Amount-based unique payment confirmations

One area concern for payment confirmation is the case in which a merchant is receiving frequent payments of the same exact amount from multiple customers. 

In this case, it can be hard to accurately identify which payments belong to which customers, and there is a possibility for a malicious customer to provide the merchant wallet with a Nano block hash paid by another user before the other user's payment was verified by the merchant wallet.

One possible solution to this would be for the merchant wallet client to reduce the payment amount by a randomized tiny amount to provide a unique transaction amount to be used as a unique identifier for the payment.

For example, if the payment total amounts to 10 XNO, the merchant wallet client could generate a random tiny amount, such as 0.0009229 XNO to subtract from the total amount, resulting in a payment total of 9.9990771 XNO.

It would then be relatively simple to confirm that this specific payment has actually been received.

<small>Note: It's important that the amount be SUBTRACTED from the total, rather than added to the total, as this prevents any concerns with customers paying more than they should have needed to. This amount should be negligible to the merchant.

It's also worth noting that currently many popular Nano wallet clients limit the usable decimals to 6-7 digits, though the Nano protocol and nodes support a maximum of 30 decimal digits.</small>
